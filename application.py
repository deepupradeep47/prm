...
# Session(app)
# excel.init_excel(app)
# mydb=mysql.connector.connect(host='localhost',user='root',password='123456789',db='prm')
# @app.route('/')
# def index():
#     return render_template('title.html')
# @app.route('/login',methods=['GET','POST'])
...
#         data=cursor.fetchall()
#         array_data=[list(i) for i in data]
#         final_data=array_data.insert(0,columns)
#         return excel.make_response_from_array(array_data,'xlsx',filename='notesdata')
#     else:
#         return redirect(url_for('login'))
...
def registration():
    if request.method=='POST':
        ...
        try:
            ...
            flash('usernsme or emsil is already exist')
            return render_template('registration.html')
        else:
            mydb.commit()
            cursor.close()
            ...
            body=f'tq for signing up.follow this link-\n\n{confirm_link}'
            sendmail(to=email,body=body,subject=subject)
            flash('confirm link sent check your email')
            return render_template('registration.html')
    return render_template('registration.html')
@app.route('/confirm/<token>')
def confirm(token):
    try:
        ...
        if status=='confirmed':
            ...
            cursor.execute("update users set email_status='confirmed' where email=%s",[email])
            mydb.commit()
            flash('Email confirmation sucess')
            return redirect(url_for('login'))
